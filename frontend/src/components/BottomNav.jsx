...
const BottomNav = ({ isDark }) => {
  ...
  const isFoodPartnerProfile = path.startsWith('/food-partner/') &&
    path !== '/food-partner/login' &&
    path !== '/food-partner/register';
  const profilePath = isFoodPartnerProfile ? path : '/user/profile';
  ...
  return (
    <nav className={`bottom-nav ${isDark ? 'dark' : ''}`}>
      ...
      <Link to="/food-partner/login" className={`nav-item ${path === '/create-food' ? 'active' : ''}`}>
        ...
        <span>   <p> Become Partner   </p> </span> 
        
      </Link>

      <Link to= '/user/profile' className={`nav-item ${path === profilePath ? 'active' : ''}`}>
        <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
